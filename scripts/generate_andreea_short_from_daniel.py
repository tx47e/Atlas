...
def indexed_paragraphs(base: str, paragraphs: list[str], start: int = 0) -> str:
...
def andreea_conclusions() -> str:
    ...
    career = [
        ...
        "În intervalul actual, **12.01.2026–11.01.2027**, te afli în Ciclul **4** de 9 ani, în Anul **2**, cu vibrația anuală **5** și Lecția **7**. Pinaclul **1** aduce Oportunitatea **4** și Provocarea **2**, iar Soarta și Destinul sunt la **2 / 2**. Este o perioadă bună pentru reorganizare și colaborări, dar Lecția **7** îți cere să cercetezi temeinic, să îți aprofundezi competențele și să nu iei decizii importante înainte de a înțelege bine direcția.",
    ...
    relationship = [
        ...
        "În prezent, până la 12.01.2027, lecția ta principală este **7**. Ea îți cere introspecție, răbdare și înțelepciunea de a înțelege ce se petrece dincolo de reacțiile de moment. În relație, oferă-ți timp pentru reflecție și repaus, dar nu transforma nevoia de liniște în izolare; adu în dialog concluziile la care ajungi.",
        ...
        "Ai grijă la impulsul de a prelua conducerea întregii relații. Anul **2** cere cooperare, iar lecția **7** îți cere introspecție, răbdare și claritate înaintea deciziilor. Spune ce ai nevoie, ascultă răspunsul și lasă-l pe Daniel să își asume partea lui fără să îi scrii tu rolul.",
    ...
    closing = [
        "Până la **12.01.2027**, te afli sub Lecția **7**. Pentru tine, Andreea, aceasta este o perioadă potrivită să te apropii de Dumnezeu, să înțelegi mai profund spiritualitatea și să descoperi ce îți spune propria viață interioară. Poți începe un proiect nou, însă energia lui **7** nu îți cere zgomot sau grabă, ci cercetare, analiză și pregătire atentă. Este un timp bun pentru introspecție, studiu și pentru a deveni mai înțeleaptă prin ceea ce observi și trăiești.",
        "Lecția **7** te invită să pregătești în liniște următoarea etapă a vieții tale și să înțelegi mai clar cine ești cu adevărat în interior. Nu ai nevoie să demonstrezi imediat ceea ce construiești. Alege momente de repaus, retrage-te din agitația inutilă și lasă răspunsurile să se așeze. Cu cât dobândești mai multă înțelepciune și discernământ, cu atât vei putea recunoaște mai ușor colaborările și relațiile bune pe care această perioadă le poate aduce.",
        "Până la vârsta de **32 de ani**, Provocarea **2** te învață să îți depășești timiditatea și să ai încredere în prezența ta în relație cu ceilalți. Ai de exersat colaborarea, comunicarea și capacitatea de a primi ajutor, fără să simți că astfel îți pierzi independența. Tot această provocare te invită să îți cultivi feminitatea, receptivitatea și delicatețea, să accepți oamenii ca oglinzi și să înțelegi dualitatea: uneori adevărul tău se clarifică tocmai prin întâlnirea cu perspectiva celuilalt.",
        "În același interval, Oportunitatea **4** îți oferă contexte în care să muncești, să devii mai organizată și să înveți cum se construiește o fundație stabilă. Poți fi susținută de medii bine structurate — cadrul medical, cadrul organizațional, instituții, echipe sau orice activitate în care există reguli, responsabilități și continuitate. Lecția nu este să devii rigidă, ci să transformi ordinea într-un sprijin: să planifici, să respecți etapele și să duci lucrurile până la capăt.",
        "În jurul vârstei de **31 de ani**, începi să intri tot mai conștient pe Destinul tău **31/4**. Muntele tău de urcat în această viață este muntele constructorului: omul-pilon, cel pe care ceilalți se pot baza, care creează structuri, metode și tipare sănătoase și care nu lasă lucrurile făcute de mântuială. Destinul **4** îți cere seriozitate, răbdare și calitatea de a transforma o idee într-o realitate trainică. Puterea ta nu stă doar în ceea ce începi, ci mai ales în ceea ce așezi bine și poți susține în timp.",
        "Privite împreună, Lecția **7**, Provocarea **2** și Oportunitatea **4** îți arată o direcție foarte clară: mai întâi îți aduni înțelepciunea în liniște, apoi înveți să o pui în relație cu oamenii și, în cele din urmă, îi dai o formă concretă. Nu te grăbi să fii văzută înainte să fii pregătită. Apropie-te de Dumnezeu, studiază, ascultă, colaborează și construiește temeinic. Astfel, intrarea pe Destinul **31/4** nu va fi doar o schimbare de etapă, ci începutul unei vieți în care devii un reper de stabilitate, seriozitate și încredere pentru tine și pentru ceilalți.",
    ]

    relationship_section = relationship[:21]
    if len(career) != 18 or len(relationship_section) != 21 or len(closing) != 6:
        raise AssertionError(
            f"Structură concluzii invalidă: {len(career)} / {len(relationship_section)} / {len(closing)}"
        )
    return "\n\n".join([
        ...
        indexed_paragraphs("P-032", relationship_section, start=18),
        f"Index: {PREFIX}-SUB-032\n\n### 11.3. Momentul prezent",
        indexed_paragraphs("P-033", closing),
    ])
...
def main() -> None:
    ...
    blocks: dict[str, str] = {
        ...
        "P-023c": "Energiile **2**, **3** și **8** sunt prezente în data ta, dar nu în matricea numelui. Sensibilitatea, comunicarea spontană și responsabilitatea există nativ, însă nu sunt întotdeauna susținute de imaginea pe care o proiectează numele. Spune clar ce simți și ce îți asumi, ca oamenii să poată vedea și aceste părți ale tale.",
        ...
        "P-028a": "În **2026**, lecția ta este **7**: introspecție, analiză, studiu, înțelepciune și repaus conștient. Ea se întâlnește cu vibrația anuală **5**, care aduce schimbare și mobilitate. Tocmai de aceea, nu te grăbi să urmezi fiecare direcție nouă; acordă-ți timp să observi, să cercetezi și să înțelegi ce merită continuat. Schimbarea devine folositoare când este ghidată de claritate interioară, nu de neliniște.",
        "T-007": cycle9_table(),
...
| **Index: RAM-19980112-v1.00r-P-047**<br>Îndrăgostiții aduc talentul alegerii, al colaborării și al construirii unor alianțe bazate pe valori comune. Profesional, te susțin în consiliere, comunicare, negociere, educație, resurse umane, servicii pentru oameni, artă, frumusețe și proiecte în care trebuie armonizate interese diferite.<br><br>Resursa ta este să vezi relația dintre oameni și să creezi acord. Umbra este indecizia sau alegerea făcută numai pentru a păstra armonia. Succesul apare când formulezi criterii clare, alegi la timp și rămâi loială valorilor care au stat la baza deciziei. | **Index: RAM-19980112-v1.00r-P-048**<br>Eremitul ca obstacol nu arată lipsa competenței, ci riscul de a lucra prea mult singură, de a analiza până când oportunitatea trece sau de a aștepta certitudinea completă. Poți acumula cunoaștere valoroasă fără să o expui suficient.<br><br>Cheia este să păstrezi profunzimea, dar să creezi termene de ieșire în lume: publicare, prezentare, ofertă, cerere de feedback. Singurătatea devine atelier, nu ascunzătoare, iar experiența ta poate deveni reper pentru ceilalți. |""",
        ...
        "P-032a": "În intervalul actual, **12.01.2026–11.01.2027**, ești în Ciclul **4** de 9 ani, Anul **2**, cu vibrația anuală **5** și Lecția **7**. Ciclul de 7 ani **5** începe, Ciclul de 12 ani **3** susține expansiunea, iar Pinaclul **1** aduce Oportunitatea **4** și Provocarea **2**. Soarta și Destinul sunt la **2 / 2**, sub zona de confort. Este o fereastră de schimbare negociată, în care analiza, aprofundarea și alegerea colaboratorilor potriviți trebuie să preceadă acțiunea.",
        "T-020": "| Fereastră | Suprapunerea principală | Utilizare profesională |\n| --- | --- | --- |\n| **12.01.2026–11.01.2027** | Ciclul de 9 ani C4, Anul **2**; vibrația **5**; Lecția **7**; Pinaclul **1**, Oportunitatea **4**, Provocarea **2**; Soartă–Destin **2 / 2** | Analiză, aprofundare, reorganizare, colaborări și testarea atentă a unei direcții noi. |\n| **12.01.2029–11.01.2030** | Vibrația **8**; Lecția **3**; Soartă–Destin **9 / 9**; Pinaclul **1** | Fereastră puternică pentru bani, vizibilitate, negociere și monetizarea creativității. |\n| **12.01.2032–11.01.2033** | Anul **8** al ciclului; vibrația **2**; Lecția **6**; Soartă–Destin **8 / 8**; Pinaclul **2**, Oportunitatea **3**, Provocarea **6** | Parteneriat profitabil, consolidare și extinderea unei activități deja validate. |\n| **12.01.2034–11.01.2035** | An important interior și exterior; începe Ciclul de 12 ani **4**; Pinaclul **2** | Repoziționare majoră și construirea unei structuri profesionale mai ample. |",
