...
MD_PATH = DIR / "2020-03-15-VULCU-REBECA-ANDREEA-scurt-v1.00r.md"
HTML_PATH = DIR / "2020-03-15-VULCU-REBECA-ANDREEA-scurt-v1.00r.html"
...
def convert_callouts(body: str) -> str:
    def repl(match: re.Match[str]) -> str:
        inner = match.group(1)
        inner = re.sub(r"<p>\[!example\]\s*([^\n<]+)\n", r"<p><strong>\1</strong><br>", inner, count=1)
        return f'<aside class="callout example">{inner}</aside>'
    return re.sub(r"<blockquote>(.*?)</blockquote>", repl, body, flags=re.S)


def apply_component_classes(body: str) -> str:
    escaped_prefix = re.escape(PREFIX)
    body = re.sub(
        rf'(<div class="idx">Index: {escaped_prefix}-T-015</div>\s*)<div class="table-wrap"><table>',
        r'\1<div class="table-wrap"><table class="twelve-year-table">',
        body,
        count=1,
    )
    body = re.sub(
        r'<tr>(?=\s*<td[^>]*><span class="active-cycle-cell")',
        r'<tr class="active-cycle">',
        body,
        count=1,
    )
    body = re.sub(
...
def wrap_tarot_figures(body: str) -> str:
    return re.sub(r'<p>(<img\b[^>]+>)</p>\s*<p><em>(.*?)</em></p>', r'<figure>\1<figcaption>\2</figcaption></figure>', body, flags=re.S)
...
def main() -> None:
    ...
    body = apply_component_classes(body)
    body = wrap_tarot_figures(body)
    body = embed_sources(body)

    css = style.group(1) + "\nblockquote{padding:14px 18px;margin:16px 0;background:linear-gradient(100deg,rgba(239,224,189,.72),rgba(255,248,232,.84));border:1px solid var(--line);border-left:5px solid var(--gold);border-radius:0 5px 5px 0} figure{margin:18px 0} figcaption{text-align:center;color:var(--muted);font-style:italic} .stage-table .stage-love.current-row td:not(.current-substage),.stage-table .stage-material.current-row td:not(.current-substage){background:#fffdf8!important}.stage-table .stage-reason.current-row td:not(.current-substage),.stage-table .stage-gifts.current-row td:not(.current-substage){background:rgba(244,229,212,.42)!important}.stage-table .current-row td.current-substage{background:#dcc39b!important;font-weight:400;box-shadow:none!important;outline:none!important}"
    document = f'''<!doctype html>
<html lang="ro">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Vulcu Rebeca Andreea — 15.03.2020</title>
<style>{css}</style>
</head>
<body>
<header><div class="title">Vulcu Rebeca Andreea · lucrare numerologică scurtă</div><div class="meta">Revizie V1.00R · 08.08.2026 · The Scribe</div></header>
<main>{body}</main>
</body>
</html>
'''
