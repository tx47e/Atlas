...
section_match = re.search(
    r"### 1\.5\. Codul numerologic personal al numelui\n.*?(?=### 1\.6\. Ciclicitatile\n)",
    md,
    flags=re.S,
)
if not section_match:
    raise SystemExit("Could not locate MD section 1.5")
...
html_pattern = re.compile(
    r'<h3 id="1-5-codul-numerologic-personal-al-numelui">.*?(?=<h3 id="1-6-ciclicitatile">)',
    flags=re.S,
)
if not html_pattern.search(page):
    raise SystemExit("Could not locate HTML section 1.5")

page = html_pattern.sub(replacement, page, count=1)
HTML_PATH.write_text(page, encoding="utf-8")
print("synced section 1.5")
